/**
 Original author: Chris Tran
 Website: http://grgichtran.com
...
(function() {
  ...
  Lightbox = (function() {
    ...
    Lightbox.prototype.build = function() {
      ...
      this.$lightbox.find('.lb-prev').on('click', function() {
        ...
        if (_this.currentImageIndex === 0) {
          _this.changeImage(_this.album.length - 1, 0);
        } else {
          _this.changeImage(_this.currentImageIndex - 1, 0);
        }
        return false;
      });
    ...
    Lightbox.prototype.start = function($link) {
      ...
      this.changeImage(imageNumber, 0);
    };

    Lightbox.prototype.changeImage = function(imageNumber, rotation) {
      var $image, preloader,
        _this = this;
      ...
      preloader.onload = function() {
        ...
        if (rotation == 90 || rotation == 270) {
          $image.height(preloader.width);
          $image.width(preloader.height);
          if (_this.options.fitImagesInViewport) {
            windowWidth = $(window).width();
            windowHeight = $(window).height();
            maxImageWidth = windowWidth - _this.containerLeftPadding - _this.containerRightPadding - 20;
            maxImageHeight = windowHeight - _this.containerTopPadding - _this.containerBottomPadding - 110;
            if ((preloader.height > maxImageWidth) || (preloader.width > maxImageHeight)) {
              if ((preloader.height / maxImageWidth) < (preloader.width / maxImageHeight)) {
